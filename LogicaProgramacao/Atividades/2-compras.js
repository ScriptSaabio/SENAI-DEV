...
// Erro 1 - Preço definido como string ("150,00") → alterado para número (150.00) para permitir cálculos matemáticos corretos.
const produto = {
  nome: "Teclado Gamer",
  preco: 150.00,
  estoque: 10,
...
// Erro 2 - Função de desconto subtraía valor fixo (- 20) → ajustada para calcular 20% do valor (valor - (valor * 0.2)).
// Desconto em porcentagem
function aplicarDesconto(valor) {
  return valor - (valor * 0.2);
}

// Erro 3 - Variável preco inexistente ao chamar a função → corrigido para produto.preco.
const precoFinal = aplicarDesconto(produto.preco);


// Erro 4 - Estoque sendo subtraído por string ("um") → corrigido para número (1) para evitar erro de operação matemática.
