...
// Erro 1 - mlPorLitro definido como string ("1000") → corrigido para número (1000) para garantir operações matemáticas corretas.
const mlPorLitro = 1000;

// Erro 2 - Função converterMlParaLitro utilizava multiplicação (*) → corrigido para divisão (/) para converter ml em litros corretamente.
function converterMlParaLitro(quantidadeMl) {
  const resultado = quantidadeMl / mlPorLitro;
  return resultado;
}

// Erro 3 - Função converterLitroParaMl utilizava soma (+) → corrigido para multiplicação (*) para converter litros em ml corretamente.
...
// Erro 4 - Variável const estoqueBaixo = true; declarada mas não utilizada → comentada para evitar variável desnecessária no código.
...
// Erro 6 - Condição verificava apenas lote1 → ajustada para verificar se lote1 e lote2 estão zerados antes de exibir o alerta.
